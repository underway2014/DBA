...
export const PGKEYS =
  /(\b(select|at\s+time\s+zone|as|begin|case|when|between|then|else|end|with|coalesce|filter|=|commit|VALUES|comment|public\.|PARTITION\s+OF|FOR\s+VALUES|SERIAL|CREATE\s+TABLE|PARTITION\s+BY\s+RANGE|PRIMARY\s+KEY|RENAME\s+TO|from|order\s+by|inner\s+join|and|join|right\s+join|left\s+join|union\s+all|drop\scolumn|modify\scolumn|limit|offset|asc|desc|group\s+by|pg_terminate_backend|alter\s+table|nextval|alter|SEQUENCE|column|on|update|set|insert\s+into|delete\s+from|where|count|show\s+max_connections|sum|current_date|current_time|current_timestamp|time|now)\b|(numeric|int|date|timestamp))/gi
